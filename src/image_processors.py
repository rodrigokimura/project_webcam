...
class Blend(ImageProcessor):
    ...
    def process(self, frame: Frame) -> Frame:
        return cv2.blendLinear(frame, self.background_frame, self.mask, 1 - self.mask)
...
class SelfieSegmentation(ImageProcessor):
    ...
    def process(self, frame: Frame) -> Frame:
        mask = self.engine.process(frame).segmentation_mask  # type: ignore
        cv2.threshold(mask, 0.9, 1, cv2.THRESH_BINARY, dst=mask)  # type: ignore
        cv2.dilate(mask, np.ones((5, 5), np.uint8), iterations=1, dst=mask)  # type: ignore
        cv2.blur(mask, (10, 10), dst=mask)  # type: ignore
        return mask
